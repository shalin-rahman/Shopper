    ...
    async with tenant_transaction(pool, tenant_id) as conn:
        rows = await conn.fetch(
            ...
            SELECT id, tenant_id, category_id, sku, name_en, name_bn,
                   ...
                   barcode, qr_payload, is_active, created_at, updated_at
...
async def create_product(request: Request, body: ProductCreate, settings: SettingsDep):
    sub = subdomain_from_request(request)
    if not sub:
        raise HTTPException(status_code=400, detail="Tenant subdomain required")
    ...
    pool = _pool(request)
    tenant_id = await resolve_tenant_id(pool, sub)
    base = settings.storefront_public_base_url or ""
    bc = barcode_for_sku(body.sku)
    qr = qr_payload_for_product(base, sub, body.sku)
    ...
    async with tenant_transaction(pool, tenant_id) as conn:
        try:
            ...
            raise HTTPException(status_code=409, detail="SKU already exists for this tenant") from None
    return _row_to_out(row)


@router.patch("/{product_id}", response_model=ProductOut)
async def update_product(request: Request, product_id: UUID, body: ProductUpdate):
    sub = subdomain_from_request(request)
    if not sub:
        raise HTTPException(status_code=400, detail="Tenant subdomain required")
    ...
    async with tenant_transaction(pool, tenant_id) as conn:
        current = await conn.fetchrow(
            "SELECT sku FROM tenant_data.products WHERE id = $1",
            product_id,
        )
        if not current:
            raise HTTPException(status_code=404, detail="Product not found")
...
def _row_to_out(row: asyncpg.Record) -> ProductOut:
    d = dict(row)
    for k in ("buy_price", "sell_price", "mrp", "vat_rate_pct"):
